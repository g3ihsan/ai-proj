...
FIELD_ALIASES = {
    ...
    CSV_TYPE_DEMAND: {
        "day": ("day", "day_index", "day_id", "day_no", "date"),
        "shift": ("shift", "shift_id", "shift_name", "shift_no", "period"),
        "role": ("role", "job_role", "position", "skill", "required_role"),
        "required": (
            "required",
            "required_staff",
            "staff_required",
    ...
    CSV_TYPE_SHIFTS: {
        "shift": ("shift", "shift_id", "shift_no", "id"),
        "shift_name": ("shift_name", "name", "label", "shift_label"),
        "start_hour": (
            "start_hour",
            "start",
            "start_time",
...
TOKEN_HINTS = {
    ...
    "name": ({"name"}, {"full", "name"}),
    "roles": ({"role"}, {"roles"}, {"skill"}, {"skills"}),
    "hourly_cost": ({"hourly", "cost"}, {"hourly", "rate"}, {"pay", "rate"}),
    "max_weekly_hours": (
        {"max", "weekly", "hours"},
    ...
    "required": ({"required"}, {"staff", "required"}, {"headcount"}),
    "shift_name": ({"shift", "name"}, {"shift", "label"}, {"name"}),
    "start_hour": ({"start", "hour"}, {"start", "time"}, {"begin", "hour"}),
    "end_hour": ({"end", "hour"}, {"end", "time"}, {"finish", "hour"}),
...
def _is_availability_header(normalized_header: str) -> bool:
    return bool(
        re.fullmatch(r"available_day\d+_shift\d+", normalized_header)
